...
fn prove_something(name: &str, start: &str, rewrites: &[Rewrite], goals: &[&str]) {
    ...
    let mut run = runner.run(rewrites);

    for (i, (goal_expr, goal_str)) in goal_exprs.iter().zip(goals).enumerate() {
        println!("Trying to prove goal {}: {}", i, goal_str);
        let equivs = run.egraph.equivs(&start_expr, goal_expr);
        if equivs.is_empty() {
            panic!("Couldn't prove goal {}: {}", i, goal_str);
        } else {
            let mut explanation = run.explain_equivalence(&start_expr, &goal_expr);
            println!("Explanation: {}", explanation.get_flat_string());
        }
    }
}
